"""Billing / subscription routes — Stripe Checkout integration."""
...
async def upgrade_page(request: Request, plan: str = "growth"):
    ...
    return request.app.state.templates.TemplateResponse(
        "pricing.html", {"request": request, "user": user, "selected": plan}
    )
...
async def subscribe(request: Request, plan: str):
    ...
    if not price_id or "REPLACE" in price_id:
        return {"error": f"Price ID not configured for plan: {plan}"}
    ...
    try:
        session = _stripe2.checkout.Session.create(
